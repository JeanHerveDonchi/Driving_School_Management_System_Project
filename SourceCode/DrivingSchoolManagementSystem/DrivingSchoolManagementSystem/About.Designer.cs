namespace DrivingSchoolManagementSystem
{
    partial class About
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            rtxtAbout = new RichTextBox();
            label1 = new Label();
            SuspendLayout();
            // 
            // rtxtAbout
            // 
            rtxtAbout.Location = new Point(84, 70);
            rtxtAbout.Name = "rtxtAbout";
            rtxtAbout.Size = new Size(544, 385);
            rtxtAbout.TabIndex = 0;
            rtxtAbout.Text = "";
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.Font = new Font("Segoe UI", 18F, FontStyle.Bold, GraphicsUnit.Point, 0);
            label1.Location = new Point(258, 19);
            label1.Name = "label1";
            label1.Size = new Size(169, 32);
            label1.TabIndex = 1;
            label1.Text = "ABOUT DSMS";
            // 
            // About
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            BackColor = Color.MistyRose;
            ClientSize = new Size(711, 467);
            Controls.Add(label1);
            Controls.Add(rtxtAbout);
            Name = "About";
            Text = "About";
            Load += About_Load;
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion

        private RichTextBox rtxtAbout;
        private Label label1;
    }
}